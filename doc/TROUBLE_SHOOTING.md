...
- [VS Code 파이썬 가상환경(venv) 인식 불가 문제](#vs-code-파이썬-가상환경venv-인식-불가-문제-해결-가이드)
- [상위 폴더에서 하위 프로젝트 디버깅 실행 불가](#문제-2-상위-폴더에서-하위-프로젝트-디버깅-실행-불가)
- [프로젝트별 디버깅 설정 (launch.json)](#가이드-3-프로젝트별-디버깅-설정-launchjson)
- [Next.js 라우트 그룹과 페이지 경로 충돌 문제](#이슈-4-nextjs-라우트-그룹route-group과-페이지-경로-충돌-문제)
...
## 이슈 4: Next.js 라우트 그룹(Route Group)과 페이지 경로 충돌 문제

### 문제 상황

Next.js 프로젝트(`v15.1.0`)에서 다음과 같이 `app` 디렉토리 바로 아래의 `page.tsx`와 라우트 그룹 `(board)` 안의 `page.tsx`가 동시에 존재하는 구조를 가집니다.

-   `app/page.tsx`
-   `app/(board)/page.tsx`

라우트 그룹 `(board)`는 URL 경로에 영향을 주지 않으므로, 두 파일 모두 동일한 URL 경로인 `/`에 매핑됩니다. 일반적인 예상과 달리, `npm run build` 실행 시 경로 충돌 오류가 발생하지 않고 빌드에 성공하며, `npm run start`로 실행도 정상적으로 되었습니다.

### 원인 및 분석

직접 테스트와 분석을 통해 알아낸 Next.js v15.1.0의 라우팅 규칙은 다음과 같습니다.

1.  **`app` 디렉토리 최상위 페이지의 우선순위가 가장 높다:**
    -   `app/page.tsx`는 라우트 그룹 내의 어떤 페이지(`app/(board)/page.tsx`)와 경로가 충돌하더라도, 우선적으로 사용자에게 보여집니다.
    -   이 과정에서 빌드 에러는 발생하지 않으며, 충돌하는 다른 페이지는 묵시적으로 무시됩니다.

2.  **라우트 그룹 간의 충돌은 허용되지 않는다:**
    -   만약 `app/page.tsx`가 없는 상태에서, `app/(board)/page.tsx`와 `app/(market)/page.tsx`처럼 두 개 이상의 라우트 그룹이 동일한 경로(`/`)를 두고 충돌할 경우, Next.js는 이를 허용하지 않고 빌드 시점에 경로 중복 오류를 발생시킵니다.

결론적으로, 현재 프로젝트 구조에서 빌드가 성공하는 이유는 `app/page.tsx`가 유일한 최우선 순위 페이지로 존재하여 `app/(board)/page.tsx`와의 충돌을 Next.js가 자체적으로 해결해주고 있기 때문입니다.

### 권장 해결 방안

비록 빌드는 성공하지만, 이러한 구조는 다음과 같은 잠재적인 문제를 가집니다.

-   **유지보수성 저하:** `app/(board)/page.tsx`는 실제로는 접근할 수 없는 코드가 되어, 다른 개발자가 해당 파일을 수정하더라도 아무런 변화가 없어 혼란을 야기할 수 있습니다.
-   **의도의 불명확성:** 프로젝트의 라우팅 구조가 직관적이지 않아, URL과 파일 경로의 관계를 파악하기 어렵습니다.

가장 좋은 해결책은 **라우팅 경로를 명확하게 분리**하는 것입니다.

-   **수정 전:** `src/app/(board)`
-   **수정 후:** `src/app/board`

폴더 이름에서 괄호 `()`를 제거하여 라우트 그룹을 일반 라우트 세그먼트로 변경하면, "게시판 페이지"는 의도한 대로 `/board` URL로 명확하게 분리되어 접근할 수 있게 됩니다.